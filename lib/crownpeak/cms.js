...
const login = async () => {
    ...
    return crownpeak.login(_config.CMS_USERNAME, _config.CMS_PASSWORD, _config.CMS_SERVER || "cms.crownpeak.net", _config.CMS_INSTANCE, _config.CMS_API_KEY);
};
...
const createLibraryClass = async (name, folderId) => {
    const request = new crownpeak.Asset.CreateRequest(name, folderId, 0, crownpeak.Util.AssetType.File, crownpeak.Util.TemplateLanguageType.CSharp, 0, 0, crownpeak.Util.AssetSubType.TemplateFile);
    return crownpeak.Asset.create(request);
};
...
const createFolder = async (name, folderId, modelId) => {
    if (modelId)
        return createFolderWithModel(name, folderId, modelId);
...
const get = async (idOrPath, bustCache) => {
    if (typeof idOrPath === "number" || parseInt(idOrPath))
        return getById(idOrPath, bustCache);
...
const createOrUpdateModel = async (shortName) => {
    ...
    if (!model || !model.asset) {
        ...
        if (!folder || !folder.asset)
            throw `Unable to create model [${folderName}]`;
        // Now create a file
        ...
        if (!template || !template.asset)
            throw `Unable to find [${templateName}] folder`;
...
const createTemplateFolder = async (name, targetFolderId) => {
    const req = new crownpeak.Asset.CreateRequest(name, targetFolderId, 0, crownpeak.Util.AssetType.Folder, -1, 0, 0, crownpeak.Util.AssetSubType.TemplateFolder);
...
const createTemplate = async (name, targetFolderId) => {
    const req = new crownpeak.Asset.CreateRequest(name, targetFolderId, 0, crownpeak.Util.AssetType.Folder, -1, 0, 0, crownpeak.Util.AssetSubType.Template);
    let resp = await crownpeak.Asset.create(req);
    if (resp && resp.ResultCode == 0) {
        _assetCache[resp.asset.id] = resp.asset;
    }
...
const createTemplateHandler = async (name, templateId, handlerCode) => {
    const req = new crownpeak.Asset.CreateRequest(name, templateId, 0, crownpeak.Util.AssetType.File, 1, 0, 0, crownpeak.Util.AssetSubType.TemplateFile);
    let resp = await crownpeak.Asset.create(req)
        .then(resp => {
        const req = new crownpeak.Asset.UpdateRequest(resp.asset.id, { 'body': handlerCode }, []);
...
const processTemplates = async (templates, wrapperName) => {
    const total = templates.reduce((acc, cur) => {
        if (cur.suppressModel)
            return acc + 1;
        if (cur.suppressFolder)
            return acc + 2;
        return acc + 3;
    }, 0);
    let completed = 0;
...
const progressBar = (message, count, total, stepmessage = "", ttyonly = true, finished = false) => {
    if (!process.stdout.isTTY) {
        if (!ttyonly) {
            ...
            if (stepmessage)
                process.stdout.write(`${stepmessage}\r\n`);
        }
        return;
    }
    ...
    if (count == 0) {
        ...
        if (maxOutputSize > outputSize)
            output += " ".repeat(maxOutputSize - outputSize);
        else
            maxOutputSize = outputSize;
    }
    if (count > 0)
...
const getSiteRootPath = async () => {
    ...
    if (parseInt(siteRootPath))
        siteRootPath = await getPath(siteRootPath);
    if (siteRootPath.slice(-1) !== "/")
        siteRootPath += "/";
    return siteRootPath;
};
const getProjectPath = async () => {
    let projectPath = _config.CMS_PROJECT;
    if (parseInt(projectPath))
        projectPath = await getPath(projectPath);
    if (projectPath.slice(-1) !== "/")
        projectPath += "/";
    return projectPath;
};
const getLibraryFolder = async () => {
    ...
    if (!result || !result.asset)
        throw "Unable to find 'Library' folder";
    return result.asset;
};
const getComponentDefinitionFolder = async () => {
    ...
    if (!result || !result.asset)
        throw "Unable to find 'Component Definitions' folder";
    return result.asset;
};
const getTemplateDefinitionFolder = async () => {
    ...
    if (!result || !result.asset)
        throw "Unable to find 'Template Definitions' folder";
    return result.asset;
};
const getWrapperDefinitionFolder = async () => {
    ...
    if (!result || !result.asset)
        throw "Unable to find 'Nav Wrapper Definitions' folder";
    return result.asset;
};
const getModelsFolder = async () => {
    ...
    if (!result || !result.asset)
        throw "Unable to find 'Models' folder";
    return result.asset;
};
const getTemplatesFolder = async () => {
    ...
    if (!result || !result.asset)
        throw "Unable to find 'Templates' folder";
    return result.asset;
};
const getComponentDefinitionModel = async () => {
    ...
    if (!result || !result.asset)
        throw "Unable to find 'Enhanced Component' model";
    return result.asset;
};
const getTemplateDefinitionModel = async () => {
    ...
    if (!result || !result.asset)
        throw "Unable to find 'Enhanced Template' model";
    return result.asset;
};
const getWrapperDefinitionModel = async () => {
    ...
    if (!result || !result.asset)
        throw "Unable to find 'Wrapper' model";
    return result.asset;
};
const getWorkflowId = async () => {
    let workflow = _config.CMS_WORKFLOW || "0";
    if (parseInt(workflow) && parseInt(workflow).toString().length === workflow.length)
        return workflow;
