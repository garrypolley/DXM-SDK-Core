// @ts_check 
const fs = require('fs'), http = require('http'), https = require('https'), path = require('path'), xml2js = require('xml2js'), cms = require("./cms"), LATEST_PATCH = "../../dxm/dxm-cl-patch-for-sdk-latest.xml";
...
async function createLibraryClass(desc, importTargetFolder, fulfilledDependencies) {
    ...
    let bodyContent = Array.isArray(desc.data.fields.field)
...
async function patch(cms, source, importTargetFolderId) {
    ...
    async function create(desc) {
        let fulfilledDependencies = await Promise.all(desc.dependsOn.map(async (d) => {
    ...
    if (resp.isSuccessful) {
        ...
        if (source.indexOf("http") === 0) {
            ...
            if (!fs.existsSync(filepath))
                filepath = path.resolve(__dirname, source);
            if (!fs.existsSync(filepath))
                filepath = path.resolve(process.env.INIT_CWD || path.resolve('.'), source);
            if (!fs.existsSync(filepath)) {
                console.error(`PATCH: ERROR: Could not read the source file [${source}]`);
            }
            else {
                fs.readFile(filepath, async (_err, data) => {
                    if (data) {
